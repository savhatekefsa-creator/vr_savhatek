...
namespace AprilTag
{
    ...
    public sealed class TagDetector : System.IDisposable
    {
        ...
        public IEnumerable<Interop.Detection> RawDetections => _rawDetections;
        ...
        public TagDetector(int width, int height, int decimation = 2)
            : this(width, height, Interop.TagFamily.Tag36h11, decimation) { }

        public TagDetector(int width, int height, Interop.TagFamily family, int decimation = 2)
        {
            // Object creation
            _detector = Interop.Detector.Create();
            _family =
                family == Interop.TagFamily.Tag36h11
                    ? Interop.Family.CreateTag36h11()
                    : Interop.Family.CreateTagStandard41h12();
        ...
        List<Interop.Detection> _rawDetections = new List<Interop.Detection>();
        ...
        void RunDetectorAndEstimator(float fov, float tagSize)
        {
            ...
            // Store raw detections for bounding box visualization
            _rawDetections.Clear();
            for (var i = 0; i < tagCount; i++)
                _rawDetections.Add(tags[i]);
