...
namespace PassthroughCameraSamples
{
    public static class PassthroughCameraUtils
    {
        ...
        // META SDK'SIZ UYARLAMA: OVRPose -> Unity'nin kendi Pose'u. Ikisi de ayni seyi tutuyor
        // (konum + yon); OVRPose yalnizca bir veri kabiydi.
        ...
        /// <summary>
        /// META SDK'SIZ UYARLAMA — cihaz modelini Android'den okur (OVRPlugin yerine).
        /// Quest 3 ve 3S ailesi Passthrough Camera API'yi destekleyen modeller.
        /// </summary>
        private static bool IsQuest3Family
        {
            get
            {
                ...
                try
                {
                    ...
                    return model.Contains("Quest 3");   // "Quest 3" ve "Quest 3S" ikisini de yakalar
                }
                catch
                {
                    return true; // model okunamadi -> engelleme, asil kapi kamerayi acabilmek
                }
#else
                return false;   // Editor'de passthrough kamera yok
#endif
            }
        }
        ...
        public static bool IsSupported
        {
            get
            {
                ...
                {
                    // META SDK'SIZ UYARLAMA: ozgun kod cihaz modelini OVRPlugin'den soruyordu.
                    // Onun yerine Android'in kendi model bilgisine bakiyoruz — SDK bagimliligi
                    // yok ve ayni ayrimi yapiyor (Quest 3 / 3S). Model okunamazsa OS surumune
                    // guveniyoruz; asil kapi zaten kamera acilabiliyor mu sorusu.
                    return IsQuest3Family
        ...
        )
        {
            ...
            using var sensorSize = GetCameraValueByKey<AndroidJavaObject>(
                cameraCharacteristics,
        ...
        public static Pose GetCameraPoseInWorld(PassthroughCameraEye cameraEye)
        {
            ...
            if (s_cachedCameraPosesRelativeToHead[index] == null)
            {
                ...
                using var cameraCharacteristics = s_cameraManager.Call<AndroidJavaObject>(
            ...
            // META SDK'SIZ UYARLAMA — tek gercek bagimlilik buydu.
            // Ozgun kod: OVRPlugin.GetNodePoseStateImmediate(Node.Head)
            // Bizde kafa pozu zaten XR rig'in kamera transformunda; Unity OpenXR onu suruyor.
            // NOT: OVRPlugin "en guncel" pozu verirdi, bu ise O KARENIN render pozu. Aradaki
            // birkac ms fark duvarda sabit duran bir marker icin onemsiz; sorun cikarsa
            // timestamp hizalamasiyla duzeltilir.
            var head = VRMultiplayer.XRRigReference.HeadOrCamera;
            if (head == null)
                return new Pose(Vector3.zero, Quaternion.identity);

            var worldFromHead = new Pose(head.position, head.rotation);

            // Poz bilesimi: once kameranin kafaya gore ofseti, sonra kafanin dunya pozu.
            var worldFromCameraPos =
                worldFromHead.position + worldFromHead.rotation * headFromCamera.position;
            var worldFromCameraRot = worldFromHead.rotation * headFromCamera.rotation;

            // Ozgun koddaki eksen duzeltmesi (kamera ekseni Unity'ninkinden 180 derece donuk).
            worldFromCameraRot *= Quaternion.Euler(180, 0, 0);

            return new Pose(worldFromCameraPos, worldFromCameraRot);
        }
        ...
        {
            // META SDK'SIZ UYARLAMA: ozgun kod OVRManager'dan passthrough katmaninin acik olup
            // olmadigini soruyordu. O kontrol yalnizca ERKEN UYARI icindi — kamera erisimi
            // passthrough katmanina bagli degil, Android Camera2 uzerinden calisiyor.
            // Bagimliligi kaldirdik; kamera acilamazsa zaten asagidaki akista hata alinir.
            return true;
        }
