...
namespace PassthroughCameraSamples
{
    public class WebCamTextureManager : MonoBehaviour
    {
        ...
        {
            // META SDK'SIZ UYARLAMA: ozgun kod sahnede OVRPassthroughLayer ariyordu. Biz Meta XR
            // SDK kullanmadigimiz icin o bilesen yok — ve gerekmiyor: kamera erisimi Android
            // Camera2 uzerinden, passthrough KATMANINDAN bagimsiz calisiyor.
            if (!PassthroughCameraUtils.IsPassthroughEnabled())
            {
                PCD.DebugMessage(
                    LogType.Error,
                    "Passthrough must be enabled to use the Passthrough Camera API."
                );
                yield break;
            }
